...
const updateArticle = async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await articles_model_1.default.findById(id);
        if (!existing) {
            return res.status(404).json({ message: "Article not found" });
        }
        const { title, subtitle, content, category, tags, isPublished, isFeatured, images, coverImage, slug, excerpt, readingTime, } = req.body;
        if (!title || !content) {
            return res
                .status(400)
        ...
        // ---- published / featured ----
        const published = typeof isPublished === "string"
            ? isPublished === "true"
            : Boolean(isPublished);
        const featured = typeof isFeatured === "string"
            ? isFeatured === "true"
            : Boolean(isFeatured);
        ...
        if (Array.isArray(images)) {
            existing.images = images.map((img) => ({
                url: typeof img === "string" ? img : img.url,
                publicId: typeof img === "string"
